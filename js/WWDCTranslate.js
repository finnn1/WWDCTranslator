...
window.onload = function () {
  ...
  aud.ontimeupdate = function () {
    ...
    // 현재 시간과 가장 가까운 p 찾기
    var currentParagraphTime = 0
    var paragraphTimes = transcript.map((number, index) => {
      var paragraphTime = number.querySelector('.sentence').href.split('=').pop()
      return [index, paragraphTime - (aud.currentTime - 0.05)]
    });
    ...
    // 영어 자막을 시간에 맞게 설정하기
    if (document.getElementById('TranslateDivEng').textContent != transcript[earlierIndex].textContent) {
      document.getElementById('TranslateDivEng').textContent = transcript[earlierIndex].textContent
      document.getElementById('TranslateDiv').textContent = document.getElementById('TranslateDivEng').textContent
      console.log(transcript[earlierIndex].textContent)
    }
